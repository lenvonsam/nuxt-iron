<template lang="pug">
.select-area
  input.select-area-input(v-model="selectAreaVal", ref="selectInput", placeholder="请选择卸货地", readonly, @click.stop="selectClick")
  .select-tab-box(v-show="tabShow")
    .select-tab
      .tabs
        .head.flex
          .col.border-right-line(@click.stop="tabIndex = 0", :class="tabIndex === 0 ? 'active' : 'border-bottom-line'") 省份
          .col.border-right-line(@click.stop="tabIndex = 1", :class="tabIndex === 1 ? 'active' : 'border-bottom-line'") 城市
          .col(@click.stop="tabIndex = 2", :class="tabIndex === 2 ? 'active' : 'border-bottom-line'") 县区
      ...
      //-   b-tab.pt-10.text-blue(title="省份")
      //-     .flex.mb-10(v-for="(label, index) in category", :key="index")
      //-       .label.pt-10 {{label}}
      //-       .area-content.flex
      //-         span.area-list.pr-10.pb-10.pt-10(v-for="item in provinceList[label]", :key="item.id", @click.stop="selectProvince(item)") {{item.name}}
      //-   b-tab.pt-10.text-blue(title="城市")
      //-     .flex.mb-10(v-for="(label, index) in category", :key="index")
      //-       .label.pt-10 {{label}}
      //-       .area-content.flex
      //-         span.area-list.pr-10.pb-10.pt-10(v-for="item in cityList[label]", :key="item.id", @click.stop="selectCity(item)") {{item.name}}
      //-   b-tab.pt-10.text-blue(title="县区")
      //-     .flex.mb-10(v-for="(label, index) in category", :key="index")
      //-       .label.pt-10 {{label}}
      //-       .area-content.flex
      //-         span.area-list.pr-10.pb-10.pt-10(v-for="item in areaList[label]", :key="item.id", @click.stop="selectArea(item)") {{item.name}}
...
class Index extends Vue {
  ...
  getProvince() {
    this.provinceList = this.filterArea(this.areaData)
  }
